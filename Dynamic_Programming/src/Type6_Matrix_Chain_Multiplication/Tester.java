	...
	public static int mcmTab(int[] arr) {
		...
		for(int len=2;len<arr.length;len++) { // how many length
			...
			for(int row=1;row<=n-len;row++) {  // diagonal length decreasing
				int col = row+len-1;
				int minCost = Integer.MAX_VALUE;
				for(int k=row;k<col;k++) {   // row and col and k comes intuitu from diagram, direct solution is always confusing , so recommended to always draw the table and then figure out pattern of row, col and k
					int cost1 = dp[row][k];
